...
> **به‌روزرسانی:** نسبت‌دهی tenure گزارشات · جزئیات فارسی تاریخچه · Respite Time · پرداخت جزئی قبل از سررسید تعهد · `case_events`  
> سند مرجع مشترک تیم‌های محصول، طراحی و توسعه — دیاگرام وضعیت: [case-status-diagram.html](./case-status-diagram.html)
...
**پیاده‌سازی دمو (Respite Time):**
- هنگام افزایش مطالبات در حین `next_action_date` فعال (Respite)، رویداد **«تأخیر تغییر استراتژی (Respite Time)»** ثبت می‌شود؛ `strategy_id` تا سررسید `respite_until` عوض نمی‌شود.
- در سررسید، **`processDeferredCeiStrategyShifts`** (import + ابتدای tick موتور) تغییر سگمنت/استراتژی را اعمال و رویداد **«اعمال تغییر استراتژی معوق»** ثبت می‌کند.
- مرجع فنی: [TECHNICAL.md §۵.۴](./TECHNICAL.md#۵۴-منطق-بهروزرسانی-cei-و-تغییر-استراتژی) · [case-status-diagram.html](./case-status-diagram.html)
...
### ۵.۶.۱ پرداخت جزئی قبل از سررسید تعهد (پیاده‌سازی دمو)

اگر پرونده **تعهد پرداخت pending** دارد و مبلغ پرداخت **کمتر از مبلغ تعهد** باشد و **قبل از `promised_datetime`** ثبت شود:

- پرداخت به‌عنوان **جزئی** ثبت می‌شود (نه لزوماً تسویه کامل)
- CEI و سگمنت به‌روز می‌شوند؛ وضعیت پرونده معمولاً در **فاز مذاکره** می‌ماند
- `next_action` و `next_action_date` بر اساس منطق پرداخت جزئی تنظیم می‌شوند
- در تاریخچه با عملیات **«پرداخت جزئی قبل از سررسید تعهد»** ثبت می‌شود (جزئیات فارسی در UI)

> این حالت با «پرداخت جزئی بدهی» معمول (پس از سررسید تعهد یا بدون تعهد باز) تفاوت دارد.
...
## ۵.۱۲ منطق گزارشات — نسبت‌دهی استراتژی (tenure) و اکشن

> **پیاده‌سازی دمو:** [§۱۰.۸](#۱۰۸-صفحه-گزارشات--پیاده‌سازی-دمو) · مرجع فنی: [TECHNICAL.md §۵.۵](./TECHNICAL.md#۵۵-گزارشات-reports)

### ۵.۱۲.۱ چرا tenure و نه `strategy_id` فعلی؟

`cases.strategy_id` فقط استراتژی **فعلی** پرونده را نشان می‌دهد. برای مقایسه اثربخشی استراتژی‌ها، هر بازه‌ای که پرونده واقعاً روی یک استراتژی بوده باید جداگانه شمرده شود.

**مثال:** پرونده‌ای که ابتدا استراتژی سبک و سپس سنگین داشته، در گزارش **هر دو** استراتژی ظاهر می‌شود — نه فقط استراتژی فعلی.

تایم‌لاین tenure از **`case_events`** بازسازی می‌شود (رویدادهایی مانند: تخصیص استراتژی، به‌روزرسانی CEI و استراتژی، تغییر استراتژی پس از پرداخت جزئی، اعمال تغییر استراتژی معوق، شکست استراتژی).

### ۵.۱۲.۲ شاخص‌های عملکرد استراتژی (تب عملکرد / A/B Test)

| شاخص | تعریف |
|------|--------|
| تعداد پرونده | پرونده‌هایی که حداقل یک tenure روی آن استراتژی دارند |
| نرخ تبدیل | پرونده‌هایی که **پرداخت کامل** (`payment_type = full`) داخل همان tenure داشته‌اند ÷ تعداد پرونده |
| میانگین روز تا پرداخت | فاصله تا **اولین پرداخت کامل** داخل tenure |
| هزینه کل | مجموع هزینه اقدامات اجراشده داخل tenure |
| مبلغ وصول‌شده | مجموع **همه** پرداخت‌های جزئی و کامل داخل tenure |

> **تفکیک مهم:** نرخ تبدیل استراتژی فقط **پرداخت کامل** است. پرداخت جزئی در «مبلغ وصول‌شده» و KPIهای مالی می‌آید، نه در نرخ تبدیل.

فیلتر بازه در تب عملکرد/هزینه بر اساس **تاریخ اجرای اقدام** داخل tenure اعمال می‌شود. با انتخاب یک استراتژی در تب هزینه/وصول، همان منطق tenure (نه `strategy_id` لحظه‌ای) به کار می‌رود.

### ۵.۱۲.۳ شاخص‌های هزینه و وصول به تفکیک اکشن

| شاخص | تعریف |
|------|--------|
| تعداد اجرا | تعداد دفعاتی که آن `action_type` اجرا شده |
| تعداد وصول | تعداد **پرداخت‌ها** — هر پرداخت جدا (last-touch: آخرین اقدام تبدیل قبل از پرداخت) |
| نرخ تبدیل | تعداد **اجراهای یکتا** که حداقل یک وصول داشته‌اند ÷ تعداد اجرا |
| مبلغ وصول‌شده | مجموع مبلغ پرداخت‌های نسبت‌داده‌شده |

> **تفکیک مهم:** اگر دو پرداخت پس از **یک** اجرای اکشن ثبت شود → در «تعداد وصول» **دو** بار، در «نرخ تبدیل» **یک** بار (یک اجرای تبدیل‌کننده). tooltip ستون‌های جدول در UI این تفاوت را توضیح می‌دهد.

### ۵.۱۲.۴ جزئیات تاریخچه در UI

جزئیات عملیات در مدال/ستون تاریخچه به **فارسی** نمایش داده می‌شود (از جمله «پرداخت جزئی قبل از سررسید تعهد»). مرجع: `historyDetails.js`.
...
**عملیات‌های قابل ثبت توسط سیستم:** ایجاد پرونده، محاسبه CEI، تعیین سگمنت، تخصیص استراتژی، به‌روزرسانی CEI و استراتژی، تأخیر تغییر استراتژی (Respite Time)، انتظار پایان استراتژی فعلی، اعمال تغییر استراتژی معوق، به‌روزرسانی اطلاعات پرونده، به‌روزرسانی اطلاعات مالی پرونده، اجرای پیامک، اجرای پیامک (شبیه‌سازی)، ارسال ناموفق پیامک — تلاش مجدد، اجرای تماس خودکار، تماس خودکار ناموفق — تلاش مجدد، ارجاع به مذاکره‌کننده، بازگشت به تماس مذاکره‌کننده، عبور به اقدام بعدی استراتژی، پایان استراتژی، **شکست استراتژی**، ارجاع خودکار به حقوقی پس از رسیدن به حداکثر تماس، پرداخت کامل بدهی، پرداخت جزئی بدهی، **پرداخت جزئی قبل از سررسید تعهد**، ادامه استراتژی پس از پرداخت جزئی، تغییر استراتژی پس از پرداخت جزئی، سوخت پرونده — فوت کاربر
...
7. دسترسی: فقط ادمین.
8. **پیاده‌سازی دمو:** دکمه **دانلود فایل نمونه** برای هر نوع عملیات (فرمت TSV/Excel) — نمونه‌ها در `google-sheet-samples/`.
...
**نرخ تبدیل هر اکشن:** پیامک هشدار، پیامک تهدید، تماس خودکار هشدار، تماس خودکار تهدید، تماس مذاکره‌کننده (اول تا هر تعداد که تعریف شده باشد) — ر.ک. [§۵.۱۲.۳](#۵۱۲۳-شاخصهای-هزینه-و-وصول-به-تفکیک-اکشن)

**نرخ تبدیل هر استراتژی:** بر اساس tenure — ر.ک. [§۵.۱۲.۲](#۵۱۲۲-شاخصهای-عملکرد-استراتژی-تب-عملکرد--ab-test)
...
**Acceptance Criteria:**
1. دسترسی: فقط ادمین.
2. **نرخ تبدیل استراتژی** = پرونده‌های با پرداخت **کامل** داخل tenure ÷ تعداد پرونده‌های آن استراتژی (نه پرداخت جزئی).
3. **نرخ تبدیل اکشن** = اجراهای **یکتا** با حداقل یک وصول ÷ تعداد کل اجرا — «تعداد وصول» هر پرداخت را جدا می‌شمارد (ر.ک. §۵.۱۲).
...
| گزارش عملکرد استراتژی | بر اساس `strategy_id` فعلی | **tenure** از `case_events` — پرونده در هر استراتژی گذشته جداگانه |
| نرخ تبدیل استراتژی | پرداخت پس از اکشن | فقط **پرداخت کامل** داخل tenure |
| نرخ تبدیل اکشن | پرداخت ÷ اجرا | **اجراهای یکتا** با وصول ÷ اجرا؛ «تعداد وصول» = هر پرداخت |
| تاریخچه رویدادها | `case_history` | **`case_events`** + `case_history` (UI از events) |
| جزئیات تاریخچه | انگلیسی/JSON خام | **فارسی** در `historyDetails.js` |
...
> منطق محصول: [§۵.۱۲](#۵۱۲-منطق-گزارشات--نسبتدهی-استراتژی-tenure-و-اکشن) · سرویس: `strategy-attribution.service.js` + `reports.service.js`

### نسبت‌دهی استراتژی (tenure)

- تایم‌لاین از **`case_events`** (تخصیص/تغییر استراتژی، شکست، …)
- پرونده سبک→سنگین در **هر دو** استراتژی در گزارش عملکرد
- **نرخ تبدیل:** فقط `payment_type = full` داخل tenure
- **وصول:** همه پرداخت‌های جزئی/کامل داخل tenure
- فیلتر `strategy_id` در تب هزینه/وصول همان tenure را اعمال می‌کند

### نرخ تبدیل اکشن (تب هزینه/وصول)

| ستون UI | API | معنی |
|---------|-----|------|
| تعداد اجرا | `execution_count` | دفعات اجرای `action_type` |
| تعداد وصول | `payment_count` | هر پرداخت جدا (last-touch) |
| نرخ تبدیل | `conversion_rate` | اجراهای یکتا با ≥۱ وصول ÷ اجرا |
| مبلغ وصول | `total_collected` | مجموع مبلغ |

Tooltip ستون‌های «تعداد وصول» و «نرخ تبدیل» تفاوت بالا را در UI توضیح می‌دهد.
