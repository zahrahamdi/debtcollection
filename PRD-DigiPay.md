...
> **فرمول هزینه تماس مذاکره‌کننده:**
> هزینه تماس مذاکره‌کننده = حقوق ساعتی مذاکره‌کننده × میانگین مدت تماس (این میانگین را طبق تجربه یک مقدار ثابت باید تعریف کنیم)
...
### ۳.۵.۲ Google Sheet Manual Sync
...
| # | وضعیت | توضیح |
|---|---|---|
| ۱ | در انتظار محاسبه CEI | پرونده تازه ایجاد شده و هنوز CEI محاسبه نشده است. |
| ۲ | در انتظار تخصیص استراتژی | CEI محاسبه شده و سیستم در حال یافتن استراتژی مناسب است. |
| ۳ | در انتظار شروع استراتژی | استراتژی تخصیص یافته و سیستم منتظر رسیدن به بازه زمانی اجرای اولین اکشن است. |
| ۴ | در انتظار نتیجه پیامک | پیامک ارسال شده و سیستم منتظر نتیجه یا پایان زمان انتظار است. |
| ۵ | در انتظار نتیجه تماس خودکار | تماس خودکار برقرار شده و سیستم منتظر نتیجه است. |
| ۶ | در انتظار تخصیص به مذاکره‌کننده | استراتژی به مرحله تماس مذاکره‌کننده رسیده یا اکشن‌های خودکار به پایان رسیده‌اند. |
| ۷ | در انتظار تماس مذاکره‌کننده | پرونده به مذاکره‌کننده تخصیص یافته و منتظر برقراری تماس است. |
| ۸ | در انتظار نتیجه تماس مذاکره‌کننده | تماس ثبت شده و سیستم منتظر پرداخت در تاریخ تعهد یا تاریخ اقدام بعدی است. |
| ۹ | در انتظار تخصیص به حقوقی | فرآیند مذاکره یا استراتژی به پایان رسیده و پرونده آماده ارجاع به حقوقی است. |
| ۱۰ | پرداخت شده | مطالبات به طور کامل تسویه شده است. |
| ۱۱ | سوخت شده | پرونده به دلیل فوت بدهکار یا موارد مشابه بسته شده است. |
...
- **اقدام چهارم: تماس تلفنی مذاکره‌کننده** — این اقدام نشان می‌دهد که در این مرحله مذاکره‌کننده باید با مشتری تماس بگیرد. پس از پایان تماس، مذاکره‌کننده روی دکمه «ثبت خروجی تماس» کلیک می‌کند و مدال ثبت خروجی تماس باز می‌شود.
...
**اطلاعات جدید ساید بار:** سابقه اکشن‌های انجام‌شده روی پرونده به ترتیب زمانی، عنوان هر اقدام، تاریخ انجام اقدام، متن پیامک یا متن تماس (در صورت وجود)، نتیجه اقدام، دکمه ثبت خروجی تماس (فقط زمانی که اقدام جاری از نوع تماس تلفنی مذاکره‌کننده باشد)
...
6. برای تماس تلفنی مذاکره‌کننده، دکمه «ثبت خروجی تماس» نمایش داده می‌شود.
...
**مسیر کاربر:** مذاکره‌کننده وارد صفحه پرونده‌ها می‌شود، فیلتر مسئول پرونده را روی نام خود قرار می‌دهد و پرونده‌های تخصیص‌یافته به خود را مشاهده می‌کند. سپس پرونده‌ها را بر اساس تاریخ اقدام بعدی از قدیم به جدید مرتب می‌کند و روی اولین پرونده قابل پیگیری کلیک می‌کند. با کلیک روی پرونده، ساید بار سمت چپ باز می‌شود. در ساید بار، ابتدا اطلاعات کامل پرونده و سپس سابقه تمامی اکشن‌های انجام‌شده روی پرونده نمایش داده می‌شود. اگر اقدام جاری پرونده تماس تلفنی مذاکره‌کننده باشد، دکمه «ثبت خروجی تماس» نمایش داده می‌شود.

دکمه ثبت خروجی تماس فقط زمانی نمایش داده می‌شود که اقدام جاری پرونده از نوع تماس تلفنی مذاکره‌کننده باشد و کاربر جاری ادمین باشد یا مذاکره‌کننده مسئول همان پرونده باشد.
...
6. اگر اقدام جاری پرونده تماس تلفنی مذاکره‌کننده باشد، دکمه «ثبت خروجی تماس» نمایش داده می‌شود.
7. با کلیک روی دکمه «ثبت خروجی تماس»، مدال ثبت خروجی تماس باز می‌شود.
...
20. پس از رسیدن به حداکثر تعداد تماس تعریف‌شده در استراتژی و در صورت عدم پرداخت، وضعیت پرونده به صورت خودکار به در انتظار تخصیص به حقوقی تغییر می‌کند.
...
7. در تاریخچه پرونده با عنوان تخصیص به مذاکره‌کننده ثبت می‌شود.
8. دسترسی: فقط ادمین.
...
5. پس از گذشت Respite Time، در صورت عدم پرداخت، اکشن بعدی اجرا می‌شود.
...
**عملیات‌های قابل ثبت توسط سیستم:** ایجاد پرونده، محاسبه CEI، به‌روزرسانی CEI، تخصیص استراتژی، تغییر استراتژی، ارسال پیامک هشدار، ارسال پیامک تهدید، تماس خودکار هشدار، تماس خودکار تهدید، به‌روزرسانی اطلاعات پرونده، ارجاع خودکار به حقوقی، پرداخت کامل بدهی، پرداخت جزئی بدهی، سوخت شدن پرونده

**عملیات‌های قابل ثبت توسط کاربر:** تخصیص به مذاکره‌کننده، تخصیص مجدد، ثبت خروجی تماس، ارسال لینک پرداخت دستی، ارسال پیامک پس از تماس بی‌پاسخ، ارجاع به حقوقی توسط مذاکره‌کننده
...
- **برای SMS:** متن پیامک با Placeholder، بازه زمانی مجاز ارسال، زمان انتظار برای اکشن بعدی (به روز)، هزینه هر پیامک
- **برای Autocall:** محتوای تماس خودکار، بازه زمانی مجاز تماس، زمان انتظار برای اکشن بعدی (به روز)، هزینه هر تماس
- **برای Negotiator Call:** بازه زمانی مجاز تماس، زمان انتظار برای اکشن بعدی (به روز)، حداکثر تعداد دفعات تکرار، میانگین مدت تماس، هزینه تماس بر اساس حقوق ساعتی مذاکره‌کننده محاسبه می‌شود

> **فرمول هزینه تماس:** هزینه تماس = حقوق ساعتی مذاکره‌کننده × میانگین مدت تماس
> حقوق ساعتی از اطلاعات مذاکره‌کننده خوانده می‌شود و در زمان ثبت تماس، هزینه همان تماس محاسبه و روی تاریخچه و هزینه پرونده ثبت می‌شود.

**Acceptance Criteria:**
1. امکان افزودن چند اکشن با دکمه + وجود دارد.
...
6. پس از انجام آخرین اکشن، در صورت عدم پرداخت، وضعیت پرونده به صورت خودکار به در انتظار تخصیص به حقوقی تغییر می‌کند.
...
**تعداد پرونده در هر وضعیت:** در انتظار نتیجه پیامک، در انتظار نتیجه تماس خودکار، در انتظار تخصیص به مذاکره‌کننده، در انتظار تماس مذاکره‌کننده، در انتظار نتیجه تماس مذاکره‌کننده، در انتظار تخصیص به حقوقی، سوخت شده، پرداخت شده

**نرخ تبدیل هر اکشن:** پیامک هشدار، پیامک تهدید، تماس خودکار هشدار، تماس خودکار تهدید، تماس تلفنی اول تا هر تعداد که تعریف شده باشد
...
1. در انتظار محاسبه CEI
2. در انتظار تخصیص استراتژی
3. در انتظار شروع استراتژی
4. در انتظار نتیجه پیامک
5. در انتظار نتیجه تماس خودکار
6. در انتظار تخصیص به مذاکره‌کننده
7. در انتظار تماس مذاکره‌کننده
8. در انتظار نتیجه تماس مذاکره‌کننده
9. در انتظار تخصیص به حقوقی
10. پرداخت شده
11. سوخت شده
